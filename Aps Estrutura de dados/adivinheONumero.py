...
def chuteDoPc(x):
  ...
  while feedback != 'c':
    ...
    feedback = input(f"Chutei o número {chute}, foi muito alto?(A), Baixo?(B) ou correto?(C)").lower()
    if feedback == 'h':
      maximo = chute - 1
...
def jogo():
  ...
  while continua != 0:
    ...
    """)
    continua = int(input("deseja continuar? 1 para sim e 0 para voltar ao menu! " ))
    if continua == 1:
      chute(10)
      chuteDoPc(10)
    else:
